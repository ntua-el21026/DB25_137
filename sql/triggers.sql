...
BEGIN
	...
	IF a >= 1 OR b > 0 THEN
		...
		SET MESSAGE_TEXT = 'Solo performance already filled.';
	END IF;
END;
//
DELIMITER ;
...
BEGIN
	IF (SELECT COUNT(*) FROM Performance_Artist WHERE perf_id = NEW.perf_id) > 0 THEN
		SIGNAL SQLSTATE '45000'
		SET MESSAGE_TEXT = 'Artists already assigned; cannot add band.';
	END IF;
END;
//
DELIMITER ;
...
BEGIN
	...
	IF first_artist IS NOT NULL THEN
		...
		IF shared_cnt = 0 THEN
			SIGNAL SQLSTATE '45000'
			SET MESSAGE_TEXT = 'Artists do not belong to the same band.';
		END IF;
	END IF;
END;
//
...
BEGIN
	IF EXISTS (
		...
		SIGNAL SQLSTATE '45000'
		SET MESSAGE_TEXT = 'Artist already included through band.';
	END IF;
END;
//
DELIMITER ;
...
BEGIN
	...
	IF perfBand IS NOT NULL
		...
		SIGNAL SQLSTATE '45000'
		SET MESSAGE_TEXT = 'Artist not a member of that band.';
	END IF;
END;
//
DELIMITER ;
...
-- ===========================================================
-- 2. Scheduling & participation (time, years, dates)
-- ===========================================================
...
BEGIN
	...
	SELECT	datetime, stage_id INTO d, s
	FROM	Performance
	WHERE	perf_id = NEW.perf_id;
	IF EXISTS (
		SELECT	1
		...
		SIGNAL SQLSTATE '45000'
		SET MESSAGE_TEXT = 'Artist already on another stage at that time.';
	END IF;
END;
//
DELIMITER ;
...
BEGIN
	...
	IF EXISTS (
		...
		SIGNAL SQLSTATE '45000'
		SET MESSAGE_TEXT = 'Band already on another stage at that time.';
	END IF;
END;
//
DELIMITER ;
...
BEGIN
	...
	IF (y - COALESCE(miny,y)) >= 3
		OR (COALESCE(maxy,y) - y) >= 3 THEN
		SIGNAL SQLSTATE '45000'
...
-- Trigger 10: Event dates must lie inside festival
DELIMITER //
...
BEGIN
	...
	IF NEW.start_date < s OR NEW.end_date > e THEN
		SIGNAL SQLSTATE '45000'
		SET MESSAGE_TEXT = 'Event dates outside festival.';
	END IF;
END;
//
DELIMITER ;
...
BEGIN
	...
	IF NEW.datetime < s OR NEW.datetime > e THEN
		SIGNAL SQLSTATE '45000'
		SET MESSAGE_TEXT = 'Performance outside festival dates.';
	END IF;
END;
//
DELIMITER ;

-- Trigger 12: Validate 5-30 min break between sequential performances (INSERT)
DELIMITER //
...
-- Trigger 13: One performance per stage at any moment (UPDATE)
DELIMITER //
...
-- Trigger 14: Validate 5-30 min break between sequential performances (UPDATE)
DELIMITER //
...
-- Trigger 15: Delete band when it has no members
DELIMITER //
...
-- Trigger 16: Check staffing ratios after INSERT on Works_On  
--              (skip the check while we are still adding security / support staff)
...
BEGIN
	/* mandatory-role ids */
	DECLARE rSec INT;	DECLARE rSup INT;
	...
	/* role of the staff member we just inserted */
	DECLARE vRole INT;
	SELECT	role_id INTO vRole
	FROM	Staff
	WHERE	staff_id = NEW.staff_id;

	/* If the inserted row is NOT security / support, re-check the ratio */
...
-- Trigger 17: Check ratios after DELETE on Works_On
DELIMITER //
...
-- Trigger 18: Check ratios after UPDATE on Works_On
DELIMITER //
...
-- Helper procedure (used by triggers 16-18)
DELIMITER //
...
-- Trigger 19: Hard block on overselling (active + on offer)
DELIMITER //
...
BEGIN
	...
	IF sold >= cap THEN
		SIGNAL SQLSTATE '45000'
		SET MESSAGE_TEXT = 'Capacity reached for this event.';
	END IF;
END;
//
DELIMITER ;

-- Trigger 20: Mark event full when last seat sold
DELIMITER //
...
-- Trigger 21: VIP tickets ≤10 % of capacity
DELIMITER //
...
BEGIN
	...
	IF NEW.type_id = vipType AND vip >= CEIL(cap*0.10) THEN
		SIGNAL SQLSTATE '45000'
		SET MESSAGE_TEXT = 'VIP ticket cap exceeded.';
	END IF;
END;
//
DELIMITER ;
...
-- Trigger 22: Only ACTIVE tickets can be offered for resale
DELIMITER //
...
BEGIN
	IF (SELECT status_id FROM Ticket WHERE ticket_id = NEW.ticket_id) <>
		...
		SIGNAL SQLSTATE '45000'
		SET MESSAGE_TEXT = 'Ticket not eligible for resale.';
	END IF;
END;
//
DELIMITER ;

-- Trigger 23: Review allowed only with USED ticket
DELIMITER //
...
BEGIN
	IF NOT EXISTS (
		...
		WHERE	t.attendee_id = NEW.attendee_id
			...
			AND	t.status_id   = (SELECT status_id FROM Ticket_Status
			                    	WHERE name='used' LIMIT 1)
	) THEN
		SIGNAL SQLSTATE '45000'
		SET MESSAGE_TEXT = 'Must have USED ticket to review.';
	END IF;
END;
//
DELIMITER ;

-- Trigger 24: Protect resale-offer timestamp
DELIMITER //
...
BEGIN
	IF NEW.offer_timestamp <> OLD.offer_timestamp THEN
		SIGNAL SQLSTATE '45000'
		SET MESSAGE_TEXT = 'offer_timestamp is immutable.';
	END IF;
END;
//
DELIMITER ;

-- Trigger 25: Protect resale-interest timestamp
DELIMITER //
...
BEGIN
	IF NEW.interest_timestamp <> OLD.interest_timestamp THEN
		SIGNAL SQLSTATE '45000'
		SET MESSAGE_TEXT = 'interest_timestamp is immutable.';
	END IF;
END;
//
DELIMITER ;
...
-- Trigger 26: Artist sub-genre must match an artist genre
DELIMITER //
...
BEGIN
	IF NOT EXISTS (
		...
		SIGNAL SQLSTATE '45000'
		SET MESSAGE_TEXT = 'Artist sub-genre inconsistent.';
	END IF;
END;
//
DELIMITER ;

-- Trigger 27: Band sub-genre must match a band genre
DELIMITER //
...
BEGIN
	IF NOT EXISTS (
		...
		SIGNAL SQLSTATE '45000'
		SET MESSAGE_TEXT = 'Band sub-genre inconsistent.';
	END IF;
END;
//
DELIMITER ;
...
-- Trigger 28: Delete attendee ⇒ purge tickets / resale / reviews
DELIMITER //
